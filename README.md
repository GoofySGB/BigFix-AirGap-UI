...
BES AirGap UI is a Windows executable interface for running `BESAirgapTool.exe`. It helps users create site lists, choose content options, run airgap processing, monitor live output, and collect generated Fixlet and SHA1 download content.

## Included Application

- `BESAirGapUI.exe` - Windows UI for the airgap workflow.

The UI expects `BESAirgapTool.exe` and the required runtime files to be supplied separately in the same folder as `BESAirGapUI.exe`.
